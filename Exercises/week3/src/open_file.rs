...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let destination = fs::read_link(std::path::Path::new(&fd_path)).ok()?;
        ...
        Some(
            OpenFile::new(
                Self::path_to_name(destination.to_str().unwrap()),
                Self::parse_cursor(&content).unwrap(),
                Self::parse_access_mode(&content).unwrap())
        )
    }
