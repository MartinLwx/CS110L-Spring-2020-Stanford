...
impl Process {
    ...
    pub fn list_fds(&self) -> Option<Vec<usize>> {
        ...
        let open_res = fs::read_dir(target_path).ok()?;
        
        for entry in open_res {
            if let Some(filename) = entry.ok()?.file_name().to_str() {
                let number = filename.parse::<usize>().ok()?;
                res.push(number);
            } else {
                return None
            }
    ...
    /// This function print some more information about this process
