...
fn read_file_lines(filename: &String) -> Result<Vec<String>, io::Error> {
    ...
    Ok(res) // because we want to return Result type
}
...
fn lcs(seq1: &Vec<String>, seq2: &Vec<String>) -> Grid {
    ...
    // we dont' need to initialize the `grid` to all zero, because it's what Grid::new() did :)
    for i in 0..m {
        for j in 0..n {
            if seq1[i] == seq2[j] {
                ...
                grid.set(
                    i + 1,
                    j + 1,
                    std::cmp::max(grid.get(i, j + 1).unwrap(), grid.get(i + 1, j).unwrap()),
                )
                .unwrap();
            }
        }
...
fn main() {
    ...
    let file1_lines = read_file_lines(filename1).expect("File1 is invalid");
    let file2_lines = read_file_lines(filename2).expect("File2 is invalid");

    let lcs_table = lcs(&file1_lines, &file2_lines);
    print_diff(
        &lcs_table,
        &file1_lines,
        &file2_lines,
